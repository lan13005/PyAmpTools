...
public:
 ...
 Neg2LnLikContrib( const vector< string >& args ) : MIFunctionContribution(m_minManager),
  ...
  /**
   * Can give access to all the LikelihoodCalculators so that the
   * user can compute values like the fit fractions
  */
  void attach( map< string, LikelihoodCalculator* >& likCalcMap ){
    cout << "[Neg2LnLikContrib] attaching LikelihoodCalculators" << endl;
    m_likCalcMap = likCalcMap;
  }

  /**
   * This function is used to calculate fit fractions, initially designed to
   * be used for regularization functions. Fit fractions are calculated for
   * each {reaction, LikelihoodCalculator} pair. This can result in duplication
   * if reactions are essentially copies of each other (i.e. when using multiple
   * polarizations). This is not expected to be a problem for regularization
   * functions as the strength hyperparameter is typically learned and is
   * problem-dependent.
  */
  void calcFitFractions( string reactionName, LikelihoodCalculator* likCalc );
...
protected:
  ...
  // This block of variables is used to calculate fit fractions
  // Derived classes need access to these
  // m_prodFactorArrayMap is needed by the derived destructor
  vector< double > fit_fractions;
  vector< string > termNames;
  map< string, double* > m_prodFactorArrayMap;
