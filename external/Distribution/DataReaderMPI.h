...
void DataReaderMPI<T>::resetSource()
{
  ...
  if( m_isLeader ){
    ...
    report( DEBUG, "DataReaderMPI" ) << "Resetting leader data source " << m_rank << endl;
    ...
    report( DEBUG, "DataReaderMPI" ) << "Resetting data source on process with rank " << m_rank << endl;
...
void DataReaderMPI<T>::distributeData()
{
  ...
  for( int i = 1; i < m_numProc; ++i ){
    ...
    report( DEBUG, "DataReaderMPI" ) << "Sending process " << i << " " << nEvents << " events" << endl;
    ...
    report( DEBUG, "DataReaderMPI" ) << "Waiting for acknowledge from " << i << endl;
    flush( cout );
    ...
    report( DEBUG, "DataReaderMPI" ) << "Send to process " << i << " finished." << endl;
    flush( cout );
  }
...
void DataReaderMPI<T>::receiveData()
{
  ...
  report( DEBUG, "DataReaderMPI" ) << "Process " << m_rank << " waiting for "
  << nEvents << " events." << endl;
  ...
  report( DEBUG, "DataReaderMPI" ) << "Process " << m_rank << " received "
