...
parser.add_argument('--ofile', type=str, default='emcee_state.h5', help='Output file name. Default "emcee_state.h5"')
...
parser.add_argument('--bsfolder', type=str, default='', help='MLE Bootstrapped samples ""')
...
def LogProb(
    ...
    ll = -1e7
    parameters = LoadParametersSampler.unflatten_parameters(par_values, KEY)
...
# MLE_VALUES, KEY, PAR_NAMES_FLAT  = LoadParametersSampler.flatten_parameters(SELECTED_VALUES)
MLE_VALUES, KEY, PAR_NAMES_FLAT  = LoadParametersSampler.flatten_parameters()
...
fig = corner.corner(samples, labels=labels_ReIm, **corner_kwargs, hist_kwargs={'density': True})
plot_axvh_fit_params(map_estimate, color='royalblue')
plot_axvh_fit_params(MLE_VALUES, color='tab:green')

################# LOAD MLE BOOTSTRAP RESULTS #################
def loadBSparameters(files):
    LoadParametersBS = LoadParameters()
    nFits = len(files)
    fitResults = FitResults(files[0])
    LoadParametersBS.load_cfg( fitResults )
    flat_params, key, parNames = LoadParametersBS.flatten_parameters()
    parameters = np.empty( (nFits, len(flat_params)) )
    statuses    = np.empty( nFits )
    parameters[0] = flat_params
    statuses[0]    = fitResults.lastMinuitCommandStatus()
    for i, file in enumerate(files[1:]):
        fitResults = FitResults(file)
        LoadParametersBS.load_cfg( fitResults )
        flat_params = LoadParametersBS.flatten_parameters()[0]
        parameters[i+1] = flat_params
        statuses[i+1] = fitResults.lastMinuitCommandStatus()
    return parameters, parNames, statuses


print(' =================== BOOTSTRAP RESULTS =================== ')
files = glob.glob(f'{bsfolder}/*result.fit')
files = sorted(files)
assert( len(files) > 1 )
